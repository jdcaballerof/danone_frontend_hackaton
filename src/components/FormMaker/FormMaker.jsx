 ...
 * @param {array<objects>} inputs * array creado en un helper y se envia por Props al formMaker
 ...
 * @param {object} defualtValues la Key de cada propiedad debe coincidir con el id del input para colocar ahi el Value
 ...
 */
export const FormMaker = ( { inputs, setSubmitObj=false, getAndsetValues=false, deps4getAndsetValues=[], callBackForSubmit=false, newProps=[], defualtValues={}, forButton, boton2 }) => {
  ...
  const { handleSubmit, control, formState:{errors}, setValue, getValues, reset, watch, resetField, setError, clearErrors } = useForm({defaultValues:defualtValues})
  ...
  return (
    ...
    <form onSubmit={handleSubmit(myHandleSubmit)} className={ `${boton.flex} ` } >
        ...
        <div className={`${boton.posicion} my-5 ${ Boolean(boton2) && 'flex flex-wrap justify-content-center '} `}>
          <Button label={boton.label} icon={boton.icon} iconPos={boton.iconPosicion} className={`p-button-${boton.estilo} hover:border-green-400 hover:bg-green-200 focus:border-green-400
focus:bg-green-200 `} onClick={handleSubmit(myHandleSubmit)} />
          {
            Boolean(boton2)
...
FormMaker.propTypes = { 
  ...
  defualtValues: PropTypes.shape({id: PropTypes.oneOfType([ PropTypes.string, PropTypes.number ]) }),
